...
@property (retain, nonatomic) NSFetchedResultsController    *fetchedResultsController_Bank;
...
//    currentLocation = nil;
    ...
    [self.bankBtn setTitle:STRING_ALL_BANK forState:UIControlStateNormal];
    ...
    r.size.height = HEIGHT_IPHONE - HEIGHT_STATUS_BAR;
...
-(void)updateListBank
{
    _bankName2BankID = [[NSMutableDictionary alloc] initWithCapacity:self.fetchedResultsController_Bank.fetchedObjects.count];
    for (BankModule *bank in self.fetchedResultsController_Bank.fetchedObjects)
    {
...
-(NSFetchedResultsController *)fetchedResultsController_Bank
{
    if (_fetchedResultsController_Bank) {
        return _fetchedResultsController_Bank;
    }
    
    NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"BankModule" inManagedObjectContext:[[AppViewController Shared] managedObjectContext]];
    [fetchRequest setEntity:entity];
    
    
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"bankNameEN"  ascending:YES selector:@selector(compare:)];
    [fetchRequest setSortDescriptors:[NSArray arrayWithObject:sortDescriptor]];
...
-(void)insertBanksFromServer:(NSDictionary*)dataDic
{
    ...
    NSArray *listBankNameVN = [[dataDic objectForKey:@"_configuration"] objectForKey:@"bankNameENList"];
    NSArray *listBankIDs = [[dataDic objectForKey:@"_configuration"] objectForKey:@"bankIDList"];
    for (NSInteger i = 0; i < listBankIDs.count; i++) {
        NSString *bankID = [listBankIDs objectAtIndex:i];
        // check exist
        BOOL isExisted = NO;
        for (BankModule *item in self.fetchedResultsController_Bank.fetchedObjects)
        {
            if ([item.bankID isEqualToString:bankID]) {
                isExisted = YES;
                break;
            }
        }
        ...
        // if it is NOT existed, insert to DB
        if (!isExisted) {
            BankModule *bank = [NSEntityDescription insertNewObjectForEntityForName:@"BankModule" inManagedObjectContext:[[AppViewController Shared] managedObjectContext]];
            bank.bankID = bankID;
...
//        //NSLog(@"Direction = %@", dicJson);
    ...
    {
        [self insertBanksFromServer:dicJson];
...
- (IBAction)bankBtnTouchUpInside:(id)sender {
    ...
    if (self.bankTableView.contentOffset.y == -100) {
